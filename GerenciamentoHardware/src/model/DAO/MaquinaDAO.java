...
public class MaquinaDAO {
    ...
    public void inserirMaquina(Maquina maquina) {
        String sql = "INSERT INTO maquinas (numero_serie, processador, ram, armazenamento, data_aquisicao, localizacao, status, laboratorio_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

        try (Connection conn = ConexaoDAO.getConnection();
                PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, maquina.getNumeroSerie());
            stmt.setString(2, maquina.getProcessador());
            stmt.setString(3, maquina.getMemoria_RAM());
